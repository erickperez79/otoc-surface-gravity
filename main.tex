% ============================================================
% PAPER V — KAELION OTOC KB SERIES
...
AdS$_2$ geometry in the IR ($R \to -2$ as $\lambda \to 0$), with all
generalizations $\beta \propto \lambda^a(1-\lambda)^b$ with
...
point. The exponent $c$ spans $c \in [0.17, 4.30]$ and is invariant under
...
The logistic $\beta$-function $\beta(\lambda) = -c\lambda(1-\lambda)$ defines
an emergent 2D metric via the RG flow~\cite{Kim2025}:
\begin{align}
ds^2 &= -A(\lambda)\,dt^2 + B(\lambda)\,d\lambda^2, \\
A(\lambda) &= A_0\left(\frac{\lambda}{1-\lambda}\right)^{2/c}, \quad
B(\lambda) = G_0\,\frac{(1-\lambda)^2}{\lambda^2}.
\end{align}

The scalar curvature satisfies $R(\lambda\to 0) = -2$ exactly for all
$c > 0$, consistent with AdS$_2$ with radius $L = c\sqrt{G_0}$.

\begin{figure}[t]
\includegraphics[width=\columnwidth]{fig3_curvature_R}
\caption{Scalar curvature $R(\lambda)$ from the RG flow metric for $c=1.0$
(left) and $c=0.5$ (right). In both cases $R\to -2$ as $\lambda\to 0$,
consistent with AdS$_2$. The dashed red line marks $R=-2$.}
\label{fig:curvature}
\end{figure}
Figure~\ref{fig:curvature} shows $R(\lambda)$ for $c = 1.0$ and $c = 0.5$.
...
generalized $\beta(\lambda) = -c\lambda^a(1-\lambda)^b$:
\begin{itemize}
\item $(a,b) = (1,1)$: $R\to -2$ in IR, AdS$_2$ clean.
\item $(a,b) = (2,1)$: $A \sim \exp(-\mathrm{const}/\lambda)$,
      super-exponential, singular.
\item $(a,b) = (1,2)$: singularity in UV ($\lambda\to 1$).
...
\caption{Scalar curvature $R(\lambda)$ for generalized
$\beta(\lambda)=-c\lambda^a(1-\lambda)^b$ with $c=1$.
...
$\lambda \leftrightarrow e^{-\phi/\phi_0}$ where $\phi$ is the JT dilaton,
...
($a_\SYK = 0.89$) and the Bose-Hubbard sonic horizon ($a_\BH = 0.235$),
with ratio $\approx 4$. The functional form is universal; the coefficient
is model-dependent.
...
corrections $\beta \to \beta(1 + \varepsilon f(\lambda))$ produce NAdS$_2$
